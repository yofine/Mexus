...
8. Write each agent section with:
   ...
   - explicit agent identity at the start of both activation and initial prompts
   - explicit kanban-first assignment rule
   - reassignment rule for tasks outside the agent's responsibility
   - Squad Lead clarification fallback when the correct owner is unclear
9. Publish initial kanban tasks under `To Claim`.

Agent activation and initial prompts must make clear:

- Both activation and initial prompts must start by naming the agent and its responsibility in the Mission.
- The agent should prioritize work assigned to `To: <AgentName>` in `kanban.md`.
- The initial prompt describes the first assigned task, not all future work in that responsibility area.
- Later kanban tasks may be assigned to the same agent within its responsibility boundary.
- If an assigned task does not fit the agent's responsibility, the agent may update `kanban.md` to reassign it to the appropriate Mission Agent and record a brief reason in `Updated`.
- If the agent cannot confidently identify the correct owner, it should publish a clarification task to `Squad Lead` instead of guessing.
